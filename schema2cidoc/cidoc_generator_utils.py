...
def build_biblio_facts(facts_g, doc_id: str, input_entry: Dict[str, Any]) -> None:
    ...
    title = dm.get("title")
    if title:
        facts_g.add((doc, DC.title, Literal(title)))
    date = dm.get("date")
    if date:
        facts_g.add((doc, DC.date, Literal(date)))
    # Authors
    ...
    for pid in creator_ids:
        facts_g.add((doc, DC.creator, EX[pid]))
    # Container
    if doc_type == "journal_article":
        ...
        if jtitle:
            facts_g.add((vol_node, DC.title, Literal(jtitle)))
        if vol:
        ...
        if jtitle:
            facts_g.add((jnode, DC.title, Literal(jtitle)))
    elif doc_type == "book_chapter":
        ...
        if btitle:
            facts_g.add((book_node, DC.title, Literal(btitle)))
...
def generate_cidoc_trig(doc_dir: str, relations_payload: Dict[str, Any], input_entry: Dict[str, Any], file_id: str) -> str:
    ...
    facts_events = gen_facts.generate_event_rdf_from_data(facts_ws, include_prefixes=False, emit_entities=False).strip()

    # Assertions: events only
    ...
    assertion_graph = gen_assert.generate_event_rdf_from_data(assertions_ws, include_prefixes=False, emit_entities=False).strip()
    ...
    # Prepare a Turtle header for parsing
    header = "\n".join([
    ...
    if facts_events:
        facts_g.parse(data=header + facts_events, format="turtle")
    if assertion_graph:
        assert_g.parse(data=header + assertion_graph, format="turtle")
